...
std::string Node::write_code_file(const long id, const mapreduce::CodeExt ext, const std::string code)
{
  ...
  const std::string filename{fmt::format("tmp/{}{}", _own_uri, get_extension_from_type(ext))};
  std::ofstream stream;
...
void Node::start_code_file(const mapreduce::CodeExt ext, const std::string code_localtion)
{
  ...
  std::string cmd{fmt::format("{} {}", code_localtion, _own_uri)};
...
{
  _console->debug("Worker {} requesting job. Waiting...", context->peer());
...
{
  _console->info("Reduce job received from: {} Waiting....", context->peer());
  std::lock_guard<std::mutex> lock(_node_mutex);
